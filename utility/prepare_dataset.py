...
# save_file_path_one_label = os.path.join(data_dir, "Consensus_Data_One_Label_By_Reflection.xlsx")
save_file_path_one_label = os.path.join(data_dir, "Consensus_Data_One_Label_all.xlsx")
...
print(label_count_overall)

# Top # Challenges Overall
# top_#_challenges = label_count_overall.head(8)
top_6_challenges = label_count_overall.head(13)

# Top # Challenges by reflection
top_6_challenges_by_reflection = label_count_by_reflection.groupby('Ref_Num').apply(
    lambda x: x[['Ref_Num', 'Challenge', 'Count']].nlargest(6, 'Count')).reset_index(drop=True)


# Create Consensus_Data_Top8_Challenges.xlsx
with pd.ExcelWriter(save_file_path_top8, engine="openpyxl") as writer:
    for ref_num, group in top_6_challenges_by_reflection.groupby('Ref_Num'):
...
with pd.ExcelWriter(save_file_path_one_label, mode="w", engine="openpyxl") as writer:
    for sheet_name in sheet_names:
        ...
        reflection_num = sheet_name[-1]
        # ref_top_challenges = top_6_challenges_by_reflection[top_6_challenges_by_reflection['Ref_Num'] == reflection_num]
